...
var BRIDGE_PREFIX = "HERMES_EXCALIDRAW:";
...
function ActionButton({ children, onClick }) {
  return jsx("button", {
    type: "button",
    className: "rounded border border-(--ui-stroke-secondary) px-2 py-1 text-xs font-medium text-(--ui-text-secondary) hover:bg-(--chrome-action-hover) focus-visible:outline-2 focus-visible:outline-(--ui-accent)",
    onClick,
    children
  });
}
function ExcalidrawPane({ ctx }) {
  ...
  return jsxs("section", {
    ...
    children: [
      ...
      showWebview ? jsx("webview", {
        ...
        role: workspace ? "status" : void 0,
        className: "grid min-h-0 flex-1 place-items-center p-4 text-center text-sm text-(--ui-text-tertiary)",
        children: workspace ? editorQuery.error ? "Editor unavailable. Use Retry to load the local asset." : "Preparing the local editor\u2026" : "Open a folder or task workspace to create hermes.excalidraw."
      }),
      showWebview && !ready ? jsx("div", {
  ...
  register(ctx) {
    ...
    const open = () => {
      ...
      close = host2.openWorkspace(ID2, {
        ...
        minWidth: "320px",
        render: () => jsx2(ExcalidrawPane, { ctx }),
        onClose: () => {
          close = null;
